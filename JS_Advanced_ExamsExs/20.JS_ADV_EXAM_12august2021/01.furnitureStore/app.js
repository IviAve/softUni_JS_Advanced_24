...
function solve() {
    ...
    const totlaPrice = Array.from(document.getElementsByClassName("total-price"))[0];
    addBtn.addEventListener("click", addOnClick)
    let sum = 0;
    function addOnClick(e) {
        e.preventDefault();

        if (modelField.value === "" || yearField.value === "" || descriptionField.value === "" || priceField.value === "" || Number(yearField.value) <= 0 || Number(priceField.value) <= 0) {
            return
        }
