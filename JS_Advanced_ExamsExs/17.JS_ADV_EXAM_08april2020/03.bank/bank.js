class Bank {
    ...
    newCustomer(customer) {
        ...
        let existingCustomer = this._allCustomers.find(c => c.personalId === personalId);
        if (existingCustomer) {
    ...
    customerInfo(personalId) {
        ...
        const info = `Bank name: ${this._bankName}
Customer name: ${customer.firstName} ${customer.lastName}
Customer ID: ${personalId}
Total Money: ${customer.totalMoney}$
Transactions:
${customer.transactions.map((transaction, index) => `${customer.transactions.length - index}. ${transaction}`).join('\n')}`;
...
// •	Otherwise this function should add the customer as a new one and return the customer details.
...
// ·	Otherwise add the amount to the corresponding customer in a property named totalMoney 
//and store the transaction information to this customer (for more clarity see the example below and the hints), 
...
// ·	Check if the given personalId corresponds to a customer in the customers array, if not throw a new error:
// “We have no customer with this ID!”
// •	If there is a customer with the given personalId, check if the customer has enough money in his account, 
//to withdraw the given amount. If the money is not enough throw a new error:
// “{firstName} {lastName} does not have enough money to withdraw that amount!”
// ·	Otherwise subtract the amount from the totalMoney  of the customer and store the transaction information
// to this customer, then return the total money of the corresponding customer and a dollar sign:
// “{totalMoney}$”
...
// •	number of transaction in descending order;
